...
namespace OBeautifulCode.Excel.Test
{
    ...
    public static partial class CommentTest
    {
                ...
                new StringRepresentationTestScenario<Comment>
                {
                    ...
                    SystemUnderTestExpectedStringRepresentationFunc = () =>
                    {
                        ...
                        var result = new SystemUnderTestExpectedStringRepresentation<Comment>
                        {
                            ...
                            ExpectedStringRepresentation = Invariant($"OBeautifulCode.Excel.Comment: Body = {systemUnderTest.Body?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, HtmlBody = {systemUnderTest.HtmlBody?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, FontName = {systemUnderTest.FontName?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, FontColor = {systemUnderTest.FontColor?.ToString() ?? "<null>"}, FontSize = {systemUnderTest.FontSize?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, FontIsBold = {systemUnderTest.FontIsBold?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, HorizontalAlignment = {systemUnderTest.HorizontalAlignment?.ToString() ?? "<null>"}, VerticalAlignment = {systemUnderTest.VerticalAlignment?.ToString() ?? "<null>"}, AutoSize = {systemUnderTest.AutoSize?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, HeightInInches = {systemUnderTest.HeightInInches?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, WidthInInches = {systemUnderTest.WidthInInches?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, FillColor = {systemUnderTest.FillColor?.ToString() ?? "<null>"}, FillTransparency = {systemUnderTest.FillTransparency?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}, BorderColor = {systemUnderTest.BorderColor?.ToString() ?? "<null>"}, BorderStyle = {systemUnderTest.BorderStyle?.ToString() ?? "<null>"}, BorderWeightInPoints = {systemUnderTest.BorderWeightInPoints?.ToString(CultureInfo.InvariantCulture) ?? "<null>"}."),
                        };
        ...
        private static readonly ValidModelTestScenarios<Comment> ValidModelTestScenarios = new ValidModelTestScenarios<Comment>()
            .AddScenario(() =>
                new ValidModelTestScenario<Comment>
                {
                    Name = "a dummy Comment should be valid",
                    SystemUnderTest = A.Dummy<Comment>(),
                });
        ...
        public static class Structural
        {
            ...
            public static void Comment___Should_implement_IModel_of_Comment___When_reflecting()
            {
                ...
                var expectedModelMethods = typeof(IModel<Comment>).GetInterfaceDeclaredAndImplementedMethods();
        ...
        public static class Cloning
        {
            ...
            public static void DeepCloneWith___Should_deep_clone_object_and_replace_the_associated_property_with_the_provided_value___When_called()
            {
                ...
                foreach (var scenario in scenarios)
                {
                    ...
                    {
                        var propertyInfo = typeof(Comment).GetPropertyFiltered(propertyName, MemberRelationships.DeclaredOrInherited, MemberOwners.Instance, MemberAccessModifiers.Public);
